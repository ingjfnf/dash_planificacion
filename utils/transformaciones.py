...
def arreglos(preclosing_df, simulacion_df, actual, historico_df):
    ...
    conjunto_total = pd.concat([preclosing_df, simulacion_df, actual_tendencia, historico_df], ignore_index=True)
...
def distributivo(df, modelo, mapeo):
    # Invertir el mapeo para filtrar por nombres originales
    mapeo_inverso = {v: k for k, v in mapeo.items()}
    
    df = df.copy()
    df["ANALISIS_ORIGINAL"] = df["ANALISIS"].replace(mapeo_inverso)

    distri = df[
        df["ANALISIS_ORIGINAL"].str.startswith("Historico_") | 
        df["ANALISIS_ORIGINAL"].isin(["ACTUAL", "FORECAST", "BUDGET"])
    ].copy()
    distri["absoluto"] = distri["VALOR"].abs()

    # --- ACTUAL: lógica de promedio mensual x 12 ---
    actual = distri[distri["ANALISIS_ORIGINAL"] == "ACTUAL"].copy()
    ...
    # --- HISTÓRICOS + BUDGET + FORECAST: lógica de suma anual ---
    series_suma_anual = distri[
        distri["ANALISIS_ORIGINAL"].str.startswith("Historico_") |
        distri["ANALISIS_ORIGINAL"].isin(["BUDGET", "FORECAST"])
    ]["ANALISIS"].unique()

    distribuciones = []
    for analisis in series_suma_anual:
        serie = distri[distri["ANALISIS"] == analisis].copy()
        suma_por_concepto = serie.groupby('CONCEPTO')['absoluto'].sum()
        serie['TOTAL CONCEPTO'] = serie['CONCEPTO'].map(suma_por_concepto)
        serie['TOTAL CONCEPTO'] = serie['TOTAL CONCEPTO'].astype("int64")
        serie['PESO PONDERADO PROMEDIO'] = serie.apply(
            lambda x: round((x["absoluto"] / x['TOTAL CONCEPTO']) * 100, 2) if x['TOTAL CONCEPTO'] != 0 else 0,
            axis=1
        )
        distribuciones.append(serie[["FECHA", "CONCEPTO", "PESO PONDERADO PROMEDIO", "ANALISIS"]])

    salida_total = pd.concat(distribuciones + [Distribucion_actual], ignore_index=True)

    # --- MODELO (desde pestaña DISTRIBUCION del archivo traza) ---
